...
//        Flux<Integer> result = Flux.empty();
        Flux<Integer> result = Flux.merge(a,b);
...
//        Flux<String> result = Flux.empty();
        Flux<String> result = Flux.zip(firstname, name).map(s -> s.getT1() + " " + s.getT2());
    ...
    public void F_flatMap() {
        ...
        Flux<String> result = source.flatMap(i -> Flux.just(i).repeat(i))
                .map(e -> map.get(e));
    ...
    public void G_subsribe()  {
        ...
        assertThat(Testing.hash("Spring5 & Reactor is so great !!"))
